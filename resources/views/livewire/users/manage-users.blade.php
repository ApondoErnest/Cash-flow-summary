<x-ui.page wide class="mf-admin-mobile-page">
    <header class="mf-admin-mobile-header mf-manage-users-header flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
    ...
    >
        ...
        @else
            <x-ui.mobile-record-list :label="trans_choice('user.manage.mobile_count', $this->users->count(), ['count' => $this->users->count()])">
                @foreach ($this->users as $user)
                    ...
                    >
                        ...
                        <x-slot:details>
                            <x-ui.mobile-record-detail :label="__('user.manage.columns.role')">
                                {{ $roleLabel($user) }}
                            </x-ui.mobile-record-detail>
                            <x-ui.mobile-record-detail :label="__('user.manage.columns.center')">
                                {{ $user->center?->name ?? '—' }}
                            </x-ui.mobile-record-detail>
                        </x-slot:details>
                        ...
                        <x-slot:actions>
                            ...
                            @if ((int) auth()->id() !== (int) $user->id)
                                ...
                                >
                                    {{ __('user.manage.actions.reset_password_mobile') }}
                                </x-ui.mobile-record-action>
                            @endif
                            ...
                            @can('delete', $user)
                                <x-ui.mobile-record-action
                                    ...
                                    wire:confirm="{{ __('user.manage.delete_confirm', ['name' => $user->name]) }}"
                                    wire:loading.attr="disabled"
                                    wire:target="deleteUser"
                                >
                                    {{ __('user.manage.actions.delete') }}
                                </x-ui.mobile-record-action>
                            @endcan
                        </x-slot:actions>
                    </x-ui.mobile-record-card>
                    ...
                    @foreach ($this->users as $user)
                        <flux:table.row wire:key="user-row-{{ $user->id }}">
                            ...
                            <flux:table.cell align="end">
                                <div class="mf-table-panel__actions inline-flex flex-wrap justify-end gap-2">
                                    <flux:button
                                        variant="ghost"
                                        size="sm"
                                        icon="pencil-square"
